...
def main():
    ...
    parser.add_argument('-cm', '--create_mode', required=False, default=False, type=bool ,help='create signature mode')
    ...
    while (True):
        ...
        if np.shape(frame) != () and not len(pallet)==3:
            ...
            if(args.create_mode):
                utils.save_pallet_input_id(pallet_vectors, save_name)
                # im_v_resize=utils.resize_frame(im_v_resize, 0.65)
                # show pallet feets 
            else:
                pallet_vectors = utils.create_vector(extractor, pallet)
                pallet_id, min_dist = utils.find_pallet_from_db(pallet_vectors)
                cv2.putText(im_v_resize, pallet_id + "[{}]".format(min_dist), (20,210), cv2.FONT_HERSHEY_SIMPLEX, 1, (10,220,220), 4)
